...
| 02 | Agenda | eight topics with their slide ranges; eight links of one chain light up in turn |
| 03 | HTTP status codes | 2xx · 3xx · 4xx · 5xx side by side, with 1xx and the `res.ok` trap underneath; 401 and 403 are tagged as the auth codes |
| 04 | Authn vs Authz | ID card scan ✓ · a keycard opens one door and is refused at the other · flip card quiz |
| 05 | RBAC matrix | permission cells cascade in; the same matrix as `PERMISSIONS` + `can()` |
| 06 | Permission engine | **live demo**: switch roles; the buttons pop or shake as permissions change |
| 07 | HTTP has no memory | two requests: the server recognises the first (token), not the second |
| 08 | Auth schemes | how every request proves who you are: stateful (session cookie → DB lookup) vs stateless (Bearer → signature check), animated; Session, JWT, OAuth 2.0, OpenID Connect, API key, mTLS and Basic compared by header, use today and status |
| 09 | Opaque token vs JWT | **step-through** with a next button: one server knows an opaque Bearer token ✓, three servers behind a load balancer don't ✕; then a JWT appears, splits into header · payload · signature, and all three servers verify it with the same key ✓ |
| 10 | JWT anatomy | header → payload → signature highlight in turn (click a part to pin it) |
| 11 | Encoding vs encryption | **toggle**: a login request crosses public Wi-Fi; over HTTP the person in the middle runs `atob()` on the Basic header and reads the password, over HTTPS they only get TLS noise; encoding, encryption, hashing and signing side by side |
| 12 | HTTP vs HTTPS | port, encryption, integrity, server identity, cookies/HSTS and HTTP/2·3 compared; below, packets pass a scanner lens and a button switches between what the middle sees over HTTP (URL, cookie, password) and over HTTPS (IP, SNI, size) |
| 13 | Access vs Refresh | timeline: access renews every 15 min ↻, refresh expires → logout |
| 14 | Where to store tokens | an XSS bug raids all four storages; httpOnly cookie shields |
| 15 | Token lifecycle | sequence diagram in sync with the six steps (click any step) |
| 16 | `fetch` wrapper | a plain request goes through `authFetch()` and comes out with `Authorization: Bearer` |
| 17 | Silent refresh | **toggle**: 4 × `/refresh` race with no lock vs one shared `refreshPromise` |
| 18 | Idle session | **live demo**: a 15 s countdown, warning phase and lock-out overlay |
| 19 | Auth state | `setUser()` → store → getters → UI badge, then `logout()` |
| 20 | Route guards | the guard sends a guest to `/login?returnUrl=…`, then lets them into `/admin` |
...
| 25 | Thank you / Q&A | near-black slide with a black-and-white padlock photo and drifting chips |
...
|---|---|---|
| `auth.js` | the current user and access token, `subscribe()`, `setUser()`, `logout()`, `can()` | 5, 19 |
| `api.js` | `authFetch()` attaches the Bearer header; `apiFetch()` handles 401 → refresh → retry | 16, 17 |
| `guards.js` · `router.js` | `authGuard`, `roleGuard`, and `navigate()`, which follows redirects | 20 |
...
| | |
|---|---|
| **HTTP is stateless** (slide 7)<br>![](docs/http-stateless.gif) | **Token lifetimes** (slide 13)<br>![](docs/token-timeline.gif) |
| **XSS vs storage** (slide 14)<br>![](docs/xss-storage.gif) | **Auth store** (slide 19)<br>![](docs/auth-store.gif) |
| **Silent refresh race** (slide 17)<br>![](docs/silent-refresh.gif) | **Route guard** (slide 20)<br>![](docs/route-guard.gif) |
| **fetch wrapper** (slide 16)<br>![](docs/fetch-wrapper.gif) | |

**Token lifecycle, step by step** (slide 15): the sequence diagram and the list advance together.
...
- **Diagrams.** Each diagram's CSS sits in the `/* ── ანიმირებული დიაგრამები ── */` block, labelled with its slide number (`/* 14 · XSS … */`).
