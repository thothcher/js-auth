...
| 06 | Auth schemes | how every request proves who you are: stateful (session cookie → DB lookup) vs stateless (Bearer → signature check), animated; Session, JWT, OAuth 2.0, OpenID Connect, API key, mTLS and Basic compared by header, use today and status |
| 07 | Encoding vs encryption | **toggle**: a login request crosses public Wi-Fi; over HTTP the person in the middle runs `atob()` on the Basic header and reads the password, over HTTPS they only get TLS noise; encoding, encryption, hashing and signing side by side |
| 08 | HTTP vs HTTPS | port, encryption, integrity, server identity, cookies/HSTS and HTTP/2·3 compared; below, packets pass a scanner lens and a button switches between what the middle sees over HTTP (URL, cookie, password) and over HTTPS (IP, SNI, size) |
| 09 | JWT anatomy | header → payload → signature highlight in turn (click a part to pin it) |
| 10 | Opaque token vs JWT | **step-through** with a next button: one server knows an opaque Bearer token ✓, three servers behind a load balancer don't ✕; then a JWT appears, splits into header · payload · signature, and all three servers verify it with the same key ✓ |
| 11 | Access vs Refresh | timeline: access renews every 15 min ↻, refresh expires → logout |
| 12 | Where to store tokens | an XSS bug raids all four storages; httpOnly cookie shields |
| 13 | Token lifecycle | sequence diagram in sync with the six steps (click any step) |
| 14 | `fetch` wrapper | a plain request goes through `authFetch()` and comes out with `Authorization: Bearer` |
| 15 | Silent refresh | **toggle**: 4 × `/refresh` race with no lock vs one shared `refreshPromise` |
| 16 | Idle session | **live demo**: a 15 s countdown, warning phase and lock-out overlay |
...
| 22 | Auth flow | **live demo**: a five-sided 3D "cube" turns left through login, register, 6-digit code, recover and new-password forms, each with live validation; the login request → response → token storage animates step by step; 8 REST endpoints, with the current form's endpoint lit |
| 23 | Big picture | a request lights up every stage of the chain |
| 24 | Common mistakes | the mistake cards appear one after another |
| 25 | Summary | gradient key phrases |
...
| `api.js` | `authFetch()` attaches the Bearer header; `apiFetch()` handles 401 → refresh → retry | 14, 15 |
...
| **XSS vs storage** (slide 12)<br>![](docs/xss-storage.gif) | **Auth store** (slide 17)<br>![](docs/auth-store.gif) |
| **Silent refresh race** (slide 15)<br>![](docs/silent-refresh.gif) | **Route guard** (slide 18)<br>![](docs/route-guard.gif) |
| **fetch wrapper** (slide 14)<br>![](docs/fetch-wrapper.gif) | **Hiding ≠ security** (slide 21)<br>![](docs/hidden-button-curl.gif) |

**Token lifecycle, step by step** (slide 13): the sequence diagram and the list advance together.
...
- **Diagrams.** Each diagram's CSS sits in the `/* ── ანიმირებული დიაგრამები ── */` block, labelled with its slide number (`/* 12 · XSS … */`).
